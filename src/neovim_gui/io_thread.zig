...
pub const IoThread = struct {
    ...
    fn ioThreadMain(self: *Self) void {
        ...
        const read_fd = self.getReadFd() orelse {
            log.err("I/O thread: no read fd available", .{});
            return;
        };

        // Event-driven loop using poll(). When Neovim sends data, poll
        // returns immediately with POLLIN so latency is the same as a
        // spin loop. When idle, the thread sleeps in the kernel instead
        // of burning 100% CPU.
        while (!self.should_stop.load(.acquire)) {
            // Process pending writes before blocking
            self.processPendingWrites() catch {};

            // Wait for data with a short timeout so we periodically
            // check should_stop and the write queue.  10ms is fast
            // enough that it's imperceptible but saves an entire core.
            ...
            if (poll_result > 0 and (fds[0].revents & std.posix.POLL.IN) != 0) {
                ...
                while (true) {
                    self.readAndProcessMessages() catch |err| {
                        switch (err) {
                            error.WouldBlock => break, // drained
            ...
            } else if (poll_result > 0 and (fds[0].revents & (std.posix.POLL.HUP | std.posix.POLL.ERR)) != 0) {
                // Peer closed or error
                log.info("Neovim connection closed (poll HUP/ERR) - sending nvim_exited event", .{});
                self.event_queue.push(.nvim_exited) catch {};
                return;
            }
            // poll_result == 0 → timeout, loop back to check writes/stop
        }
                ...
                if (args.len >= 11) {
                    if (extractF32(args[9])) |sr| {
                        if (extractF32(args[10])) |sc| {
                            ...
                            screen_col = sc;
                            use_screen_pos = true;
                            log.debug("win_float_pos: grid={} using screen_pos=({d:.1},{d:.1}) zindex={}", .{ grid, screen_col, screen_row, zindex });
                        }
                    }
                }
