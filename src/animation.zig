...
pub const CornerCursorAnimation = struct {
    ...
    /// Default base animation length (0.10 seconds - slightly faster than Neovide's 0.15)
    const DEFAULT_ANIMATION_LENGTH: f32 = 0.10;
    /// Default short animation for typing (0.04 seconds)
    const DEFAULT_SHORT_ANIMATION_LENGTH: f32 = 0.04;
    ...
    /// Returns true if still animating.
    pub fn update(self: *CornerCursorAnimation, dt: f32, cell_width: f32, cell_height: f32, config_duration: f32) bool {
        // If animations are disabled, snap all corners to destination
        if (config_duration <= 0) {
            ...
            for (&self.corners) |*corner| {
                _ = corner.update(dt, self.destination, cell_width, cell_height, true);
            }
            return false;
        }

        // Handle jump - calculate ranks and set animation lengths,
        // scaled by config_duration relative to the default (0.10s)
        if (self.jumped) {
            self.handleJump(cell_width, cell_height, config_duration);
            self.jumped = false;
        }
    ...
    fn handleJump(self: *CornerCursorAnimation, cell_width: f32, cell_height: f32, config_duration: f32) void {
        ...
        // Scale animation lengths by config_duration relative to default (0.10s).
        // e.g. config_duration=0.06 → scale=0.6, making animations 60% as long.
        const scale = config_duration / DEFAULT_ANIMATION_LENGTH;
        const base_length = if (is_small_jump)
            @min(DEFAULT_ANIMATION_LENGTH, DEFAULT_SHORT_ANIMATION_LENGTH) * scale
        else
            DEFAULT_ANIMATION_LENGTH * scale;
